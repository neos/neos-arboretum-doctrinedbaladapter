...
final class ContentSubgraph implements ContentProjection\ContentSubgraphInterface
{
 ...
 AND h.subgraphidentifier = :subgraphIdentifier',
    ...
    public function findNodesByParent(
        ...
        $query = 'SELECT c.* FROM neos_arboretum_node p
 ...
 ORDER BY h.position';
    ...
    {
        $query = 'SELECT COUNT(identifieringraph) FROM neos_arboretum_node p
 INNER JOIN neos_arboretum_hierarchyedge h ON h.parentnodesidentifieringraph = p.identifieringraph
 INNER JOIN neos_arboretum_node c ON h.childnodesidentifieringraph = c.identifieringraph
 WHERE p.identifierinsubgraph = :parentIdentifier
 AND h.subgraphidentifier = :subgraphIdentifier';
 ...
 ORDER BY h.position',
    ...
    public function findNodesByType(string $nodeTypeName, ContentRepository\Service\Context $context = null): array
    {
        $result = [];
        foreach ($this->getDatabaseConnection()->executeQuery(
            'SELECT n.*, h.name, h.index FROM neos_arboretum_node n
 INNER JOIN neos_arboretum_hierarchyedge h ON h.childnodesidentifieringraph = n.identifieringraph
